...
- **Reading** uses `MediaInfo.Wrapper.Core` on `net10.0-windows` (much more forgiving of malformed/nonstandard MP4 containers, like VLC/FFmpeg) and `TagLibSharp` on `net461` (MediaInfo doesn't support that framework). MediaInfo can't return covers/lyrics, so they're back-filled via TagLibSharp. This split lives behind `#if NET6_0_OR_GREATER` — when editing `TagService`, remember both paths exist.
